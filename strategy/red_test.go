...
func TestREDCollectors(t *testing.T) {
	t.Parallel()
	...
	red, err := NewRED(REDOpts{
		...
		RequestsOpt: REDRequestsOpt{
			RequestType:   "http",
			RequestLabels: []string{"method", "path"},
		},
		ErrorsOpt: REDErrorsOpt{
			ErrorLabels: []string{"type"},
		},
		DurationOpt: REDDurationOpt{
			DurationLabels: []string{"method", "path"},
	...
	collectors := red.Collectors()

	assert.Equal(t, 4, len(collectors))
	...
	assert.Equal(t, 1, histogramCount, "Should have 1 HistogramVec")
	assert.Equal(t, 1, summaryCount, "Should have 1 SummaryVec")
	assert.Equal(t, 2, counterCount, "Should have 2 CounterVec")
}
